...
{
	int minimap_size = 150, tile_size = minimap_size / COLS, minimap_x = SCREEN_WIDTH - minimap_size - 10, minimap_y = 10;
	...
	for (int y = 0; y < ROWS; y++)
	{
		...
		{
			SDL_SetRenderDrawColor(res->renderer, map[y][x] == 1 ? 255: 0, 255, 255, 255);
	...
	int player_minimap_x = minimap_x + (int)(cord->posX * tile_size), player_minimap_y = minimap_y + (int)(cord->posY * tile_size);
